...
import com.colin.playerdemo.adapter.MessageAdapter;
...
public class MessageFragment extends BaseFragment {
    ...
    protected void initAllMembersView(Bundle savedInstanceState) {
        adapter = new MessageAdapter(messageBeans);
    ...
    private void getMessage() {
        ...
        OkGo.<String>get(URLs.MESSAGE).params(httpParams).execute(new StringCallback() {
            @Override
            public void onSuccess(Response<String> response) {
                //解析data里面为数组的形式，用的baseListBean基本类
                Type type = new TypeToken<BaseListBean<MessageBean>>() {
                }.getType();
                BaseListBean<MessageBean> searchHotBeanBaseListBean = GsonHelper.gson.fromJson(response.body(), type);
                ...
                if (searchHotBeanBaseListBean.getResCode() == 0) {
                    ...
                    if (searchHotBeanBaseListBean.getData().isEmpty()) {
                        ...
                        messageBeans.clear();
                        messageBeans.addAll(searchHotBeanBaseListBean.getData());
